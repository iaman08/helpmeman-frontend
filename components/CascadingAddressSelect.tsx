    ...
    <div className="flex flex-col gap-5 w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        <div className="flex flex-col gap-1.5">
          <span className="text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--fg)]">
            Country
          </span>
          <SearchableSelect
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      ...
      <button
        ...
        className={`w-full flex items-center justify-between bg-[var(--fg)]/5 border ${
          isOpen ? "border-orange-500/50 ring-2 ring-orange-500/10" : "border-[var(--hairline)]/10"
        } rounded-xl px-4 py-3 text-sm focus:outline-none transition-all disabled:opacity-50 disabled:cursor-not-allowed`}
      >
        <span className={selectedItem ? "text-[var(--fg)] font-medium" : "text-[var(--fg)]"}>
          {selectedItem ? selectedItem.label : placeholder}
        </span>
        <ChevronDown
          size={15}
          className={`text-[var(--fg)] transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`}
        />
      </button>

      {/* Dropdown Panel */}
      {isOpen && !disabled && (
        <div className="absolute z-50 left-0 right-0 top-full mt-2 bg-[var(--bg)] border border-[var(--hairline)]/15 rounded-xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-1 duration-150">
          {/* Search Input inside panel */}
          <div className="relative border-b border-[var(--hairline)]/10 px-3 py-2.5">
            <Search size={14} className="absolute left-6 top-1/2 -translate-y-1/2 text-[var(--fg)]" />
            <input
              ref={inputRef}
              type="text"
              placeholder="Search..."
              ...
              className="w-full bg-[var(--fg)]/5 border border-[var(--hairline)]/5 rounded-lg pl-9 pr-4 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-orange-500/10 focus:border-orange-500/30 transition-all placeholder:text-[var(--muted)]"
            />
          </div>
          ...
          <div className="max-h-52 overflow-y-auto">
            {filteredItems.length === 0 ? (
              <div className="px-4 py-3 text-xs text-[var(--fg)] italic text-center">
                No matches found
              </div>
            ) : (
              filteredItems.map((item, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2.5 text-sm transition-colors flex items-center justify-between border-b border-[var(--hairline)]/5 last:border-0 ${
                      isSelected
                        ? "bg-orange-500/10 text-orange-500 font-bold"
                        : isActive
                        ? "bg-[var(--fg)]/5 text-[var(--fg)]"
                        : "hover:bg-[var(--fg)]/5 text-[var(--fg)]"
                    }`}
                  >
