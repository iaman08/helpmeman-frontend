...
export function MeetingCard({ booking, viewAs, onCancel, onReschedule, onRateSession, compact = false }: MeetingCardProps) {
  ...
  if (compact) {
    return (
      <Link
        ...
        className="flex items-center justify-between rounded-2xl bg-[var(--fg)]/[0.03] border border-[var(--hairline)] hover:border-[var(--hairline)]/15 hover:bg-[var(--fg)]/5 p-4 transition-all group"
      >
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[var(--fg)]/8 text-sm font-bold shrink-0">
  ...
  return (
    <div className="rounded-2xl border border-[var(--hairline)] bg-[var(--fg)]/[0.02] p-5 flex flex-col gap-4 hover:border-[var(--hairline)]/15 transition-colors">
      {/* ─── Header ─── */}
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[var(--fg)]/8 text-base font-bold shrink-0">
            {otherPersonName[0]?.toUpperCase() ?? "?"}
          </div>
          <div className="flex flex-col gap-0.5">
            <span className="font-semibold text-base">{otherPersonName}</span>
            {booking.mentor?.institutionName && viewAs === "mentee" && (
              <span className="text-xs text-[var(--fg)]">{booking.mentor.institutionName}</span>
            )}
          </div>
        </div>
        <StatusBadge status={booking.status} />
      ...
      <div className="flex flex-col gap-2 text-sm">
        <div className="flex items-center gap-2 text-[var(--fg)]/70">
          <Calendar className="h-3.5 w-3.5 text-[var(--fg)] shrink-0" />
          <span>{formatDateLocal(booking.scheduledAt, tz)}</span>
        </div>
        <div className="flex items-center gap-2 text-[var(--fg)]/70">
          <Clock className="h-3.5 w-3.5 text-[var(--fg)] shrink-0" />
          <span>{formatTimeLocal(booking.scheduledAt, tz)} · {booking.durationMinutes} min</span>
          {tz && <span className="text-xs text-[var(--fg)]">({tz})</span>}
        </div>
        {(viewAs === "mentor" && booking.user?.name) && (
          <div className="flex items-center gap-2 text-[var(--fg)]/70">
            <User className="h-3.5 w-3.5 text-[var(--fg)] shrink-0" />
        ...
        <a
          ...
          className={`flex items-center justify-center gap-2 rounded-xl py-3 text-sm font-semibold transition-all ${
            joinable
              ...
              : "bg-[var(--fg)]/8 text-[var(--fg)] cursor-not-allowed opacity-60"
          }`}
      ...
      <div className="flex items-center gap-2 pt-1">
        <Link
          href={`/dashboard/bookings/${booking.id}`}
          className="flex-1 text-center rounded-lg bg-[var(--fg)]/5 hover:bg-[var(--fg)]/10 py-2 text-xs font-medium transition-colors"
        >
          View Details
        </Link>
        ...
        {canCancel && onReschedule && (
          <button
            ...
            className="rounded-lg bg-[var(--fg)]/5 hover:bg-[var(--fg)]/10 px-3 py-2 text-xs font-medium transition-colors cursor-pointer"
          >
            Reschedule
          </button>
        )}
