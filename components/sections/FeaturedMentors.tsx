  ...
  return (
    <div className="w-full px-5 sm:px-10 lg:px-24 py-16 sm:py-24">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        ...
        <Link 
          href="/signin" 
          className="text-sm font-bold border-b border-[var(--hairline)] pb-1 hover:text-[var(--fg)] hover:border-[var(--hairline)] transition-colors"
        >
          View all mentors →
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {MENTORS.map((m) => (
          <div key={m.name} className="group relative flex flex-col gap-4">
            <div className="aspect-[4/5] w-full rounded-[2rem] overflow-hidden bg-[var(--fg)]/5 border border-[var(--hairline)]">
              <img 
                src={m.img} 
                alt={m.name} 
            ...
            <div className="flex flex-col gap-1">
              ...
              <p className="text-sm text-[var(--fg)]">{m.role}</p>
              <div className="flex flex-wrap gap-2 mt-2">
                {m.tags.map(t => (
                  <span key={t} className="text-[10px] uppercase tracking-wider font-bold bg-[var(--fg)]/5 px-2 py-0.5 rounded-full">
                    {t}
                  </span>
                ))}
              </div>
