...
const SLOT_CLASS =
  "bg-[#F4F4F9] dark:bg-[#262626] border border-black/5 dark:border-white/10 text-center font-medium text-transparent caret-transparent outline-none transition-shadow duration-200 selection:bg-transparent disabled:cursor-not-allowed disabled:opacity-50";
...
export function OtpInput({
  ...
  // Keep caret aligned with current focused cell
  useEffect(() => {
    if (focused !== null) {
      const cell = cells.current[focused];
      if (cell) {
        setCaretX(cell.offsetLeft + cell.offsetWidth / 2);
      }
    }
  }, [focused]);

  // Reliable autofocus on mount
  useEffect(() => {
    ...
    const typed =
      chars.length === 1
        ...
        : chars.length === 2 && chars[0] === slots[index]
          ? chars[1]
          : chars.length === 2 && chars[1] === slots[index]
            ? chars[0]
            : null;
