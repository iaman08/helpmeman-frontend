...
export function Avatar({ name, url, size = "md", className = "", style }: AvatarProps) {
  ...
  return (
    <div
      className={`relative shrink-0 rounded-full overflow-hidden flex items-center justify-center border border-[var(--hairline)] select-none transition-all duration-200 ${selectedSizeClass} ${
        !url || imgError
          ? "bg-gradient-to-br from-[var(--fg)]/10 to-[var(--fg)]/20 text-[var(--fg)]/80"
          : "bg-transparent"
      } ${className}`}
      style={style}
    >
