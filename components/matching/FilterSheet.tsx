      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed bottom-0 left-0 right-0 md:left-1/2 md:-translate-x-1/2 md:max-w-xl md:bottom-6 rounded-t-3xl md:rounded-3xl z-50 max-h-[85vh] overflow-y-auto no-scrollbar shadow-2xl border-t md:border border-[var(--hairline)]"
            style={{
              background: "var(--bg)",
              color: "var(--fg)",
            }}
          >
            ...
            <div className="flex justify-center pt-3 pb-1">
              <div className="h-1 w-10 rounded-full bg-[var(--fg)]/15" />
            </div>

            {/* Header */}
            <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--hairline)]">
              <div className="flex items-center gap-2">
                <SlidersHorizontal className="h-4.5 w-4.5 text-[var(--fg)]" />
                <h3 className="text-lg font-bold font-display text-[var(--fg)]">Discovery Filters</h3>
                {activeCount > 0 && (
                  <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[var(--bg)] text-[10px] font-bold text-[var(--fg)]">
                    {activeCount}
                  </span>
                )}
              </div>
            ...
            <div className="p-6 flex flex-col gap-6 pb-28">
              ...
              <FilterSection title="Category">
                <div className="relative">
                  <select
                    ...
                    className="w-full appearance-none bg-[var(--fg)]/4 border border-[var(--hairline)] hover:border-[var(--hairline)]/20 rounded-xl px-4.5 py-3 text-sm text-[var(--fg)] outline-none focus:border-[var(--hairline)]/30 focus:bg-[var(--fg)]/8 cursor-pointer transition-all font-medium"
                  >
                    <option value="" className="bg-[var(--bg)] text-[var(--fg)]">All Categories</option>
                    {categories.map((cat) => (
                      <option key={cat.id} value={cat.slug} className="bg-[var(--bg)] text-[var(--fg)]">{cat.name}</option>
                    ))}
                  </select>
                  <ChevronDown className="absolute right-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-[var(--fg)] pointer-events-none" />
                </div>
              </FilterSection>
              ...
              <FilterSection title="Price Range (₹ / session)">
                <div className="grid grid-cols-2 gap-3.5">
                  <input
                    ...
                    className="bg-[var(--fg)]/4 border border-[var(--hairline)] hover:border-[var(--hairline)]/20 rounded-xl px-4.5 py-3 text-sm text-[var(--fg)] outline-none focus:border-[var(--hairline)]/30 focus:bg-[var(--fg)]/8 placeholder:text-[var(--muted)]/60 transition-all font-medium"
                  />
                  <input
                    type="number"
                    placeholder="Max (₹)"
              ...
              <FilterSection title="Search">
                <input
                  ...
                  className="w-full bg-[var(--fg)]/5 border border-[var(--hairline)] rounded-xl px-4 py-3 text-sm text-[var(--fg)] outline-none focus:border-[var(--hairline)]/35 placeholder:text-[var(--muted)] transition-all"
                />
              </FilterSection>
            </div>
            ...
            {/* Footer */}
            <div 
              className="sticky bottom-0 flex items-center gap-4 px-6 py-4.5 border-t border-[var(--hairline)] backdrop-blur-md"
              style={{ background: "color-mix(in srgb, var(--bg) 95%, transparent)" }}
            >
              <button
                type="button"
                onClick={handleClear}
                className="flex-1 py-3 text-sm font-bold text-[var(--fg)] hover:text-[var(--fg)] transition-colors cursor-pointer"
              >
                Clear All
              </button>
              <motion.button
                ...
                onClick={handleApply}
                whileTap={{ scale: 0.97 }}
                className="flex-[2] py-3.5 rounded-xl text-sm font-bold bg-[var(--bg)] text-[var(--fg)] hover:opacity-90 active:scale-97 cursor-pointer"
              >
...
function FilterSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-2.5">
      <p className="text-[10px] uppercase tracking-[0.2em] font-bold text-[var(--fg)]">{title}</p>
      {children}
    </div>
  );
}
...
function Toggle({
  ...
  return (
    <div
      className="flex items-center justify-between gap-3 p-3 rounded-xl cursor-pointer transition-all hover:bg-[var(--fg)]/5 border border-[var(--hairline)]"
      onClick={() => onChange(!value)}
    >
      <div>
        <p className="text-sm font-medium text-[var(--fg)]">{label}</p>
        <p className="text-xs text-[var(--fg)] mt-0.5">{description}</p>
      </div>
      <div
